...
+ ```LinkedList``` is a linked list and consequently never adjusted as ArrayLists need to, making LinkedLists suitable for write heavy applications. A drawback with linked lists in general is that the list must be traversed in order to ascertain the location of an element, making LinkedLists less desirable for read heavy tasks. LinkedList does implement the ```Deque``` interface, providing quick access to the first and last elements of the linked list.
