...
+ The view class `CProductView` (generated by the wizard for this project) is derived from the MFC class for recordsets, `CRecordView`. This class contains a public member pointer variable to `CMFC_MDB_DemoSet`.
...
## CRecordSet and CRecordView derivatives
...
As mentioned (for [this](https://github.com/jfspps/VisualStudio2005Learning/tree/main/MFC_MDB_Demo) project) a derivative of `CRecordView`, `CProductView`,
serves to display information from the recordset (through `CProductView::DoDataExchange()`). The base class `CRecordView` provides much of the key
functionality, the project derived class `CProductView` customises the functionality further.
